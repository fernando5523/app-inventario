...
describe('primera carga (sin hoja elegida): elige la que le toca y la trae por id', () => {
  it('resuelve inventario, ronda, elige la hoja en proceso y devuelve su id', async () => {
    ...
    const acc = acciones({
      activo: vi.fn(async () => ({ inventarioId: 5, estado: 'en_curso' as const, rondaActiva: 1, totalHojas: 26, ultimaRondaCerrada: null })),
  ...
  it('sin ninguna hoja asignada: motivo sin-hoja, hoja null', async () => {
    const acc = acciones({ activo: vi.fn(async () => ({ inventarioId: 5, estado: 'en_curso' as const, rondaActiva: 1, totalHojas: 26, ultimaRondaCerrada: null })), mias: vi.fn(async () => []) });
    const r = await cargarHojaActiva({ ronda: null, hojaId: null }, acc);
...
describe('con una hoja abierta: se resuelve por ID, no por número', () => {
  it('MISMA numeración en dos rondas: abre la hoja correcta por id, no por "001"', async () => {
    ...
    const acc = acciones({
      activo: vi.fn(async () => ({ inventarioId: 5, estado: 'en_curso' as const, rondaActiva: 2, totalHojas: 26, ultimaRondaCerrada: null })),
  ...
  it('EL BUG DEL CLIENTE — la ronda cambió y su hoja ya no está: hoja-vieja, hoja null (aviso), rondaVieja 2', async () => {
    ...
    const acc = acciones({
      activo: vi.fn(async () => ({ inventarioId: 5, estado: 'en_curso' as const, rondaActiva: 3, totalHojas: 26, ultimaRondaCerrada: null })),
...
/**
 * EL AJUSTE DEL AUDITOR rompió una suposición que estaba en todo el archivo:
 * que "sin ronda activa" quería decir "no hay inventario". Desde que el
 * inventario sigue ABIERTO entre la última ronda cerrada y el ajuste (y
 * durante el ajuste), `activo()` devuelve un inventario con `rondaActiva:
 * null` -- y ahí el Contador recibía "todavía no tienes hojas asignadas", que
 * lo invita a esperar una asignación que no va a llegar nunca.
 */
describe('el conteo terminó: hay inventario abierto pero ninguna ronda', () => {
  it('lo distingue de "no hay inventario", que tiene la salida opuesta', async () => {
    const acc = acciones({ activo: vi.fn(async () => ({ inventarioId: 5, estado: 'en_curso' as const, rondaActiva: null, totalHojas: 26, ultimaRondaCerrada: null })) });

    const r = await cargarHojaActiva({ ronda: null, hojaId: null }, acc);

    expect(r).toMatchObject({ inventarioId: 5, ronda: null, hoja: null, motivo: 'conteo-terminado' });
  });

  it('sin inventario sigue siendo sin-inventario: las dos no se mezclan', async () => {
    const acc = acciones({ activo: vi.fn(async () => null) });

    expect(await cargarHojaActiva({ ronda: null, hojaId: null }, acc)).toMatchObject({ motivo: 'sin-inventario' });
  });
...
/**
 * `rondaActiva` es la ÚLTIMA RONDA QUE EXISTE (`max(numeroConteo)`), no una
 * que admita conteo: el backend lo documenta así
 * (`inventarios.service.ts#InventarioActivoDto`). Durante el ajuste del
 * auditor sigue trayendo la 3, y sin mirar el estado el Contador entraba a
 * "contar" una ronda donde cada ítem le habría dado 403.
 */
describe('la fase manda sobre la ronda: durante el ajuste ya no se cuenta', () => {
  it('con el ajuste en curso corta antes de pedir las hojas, aunque venga una ronda', async () => {
    const acc = acciones({
      activo: vi.fn(async () => ({ inventarioId: 5, estado: 'ajuste_auditor' as const, rondaActiva: 3, totalHojas: 26, ultimaRondaCerrada: null })),
    });

    const r = await cargarHojaActiva({ ronda: 3, hojaId: 44 }, acc);

    expect(r).toMatchObject({ inventarioId: 5, hoja: null, motivo: 'conteo-terminado' });
    expect(acc.mias).not.toHaveBeenCalled();
  });

  it('con el conteo ya cerrado, lo mismo', async () => {
    const acc = acciones({
      activo: vi.fn(async () => ({ inventarioId: 5, estado: 'conteo_cerrado' as const, rondaActiva: 3, totalHojas: 26, ultimaRondaCerrada: null })),
    });
  ...
  it('con el inventario en curso y su ronda abierta SÍ se piden las hojas', async () => {
    ...
    expect(acc.mias).toHaveBeenCalledWith(5, 2);
  });
});
