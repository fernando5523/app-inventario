...
**El tab bar absoluto tapa el ÚLTIMO ítem de una lista larga, y un padding de CONTENIDO no lo arregla del todo** — bug real encontrado en la prueba end-to-end de liquidación (2026-09-11): "Excluir" de la última línea de ajustes activaba la pestaña Usuarios en vez de excluir la línea. Causa: en React Native (`components/navegacion/PantallaConTabs.tsx`) el tab bar cuelga con `position: absolute` del contenedor de `<Tabs>` -- React Navigation no le reserva espacio a la pantalla porque es un `tabBar` custom. Rellenar el CONTENIDO scrolleable con `paddingBottom` (dentro de `contentContainerStyle`) solo da margen una vez que se scrollea hasta el final: el `ScrollView` sigue midiendo su propio viewport hasta el borde real de la pantalla, así que con una lista lo bastante larga la última fila nace ya tapada por la barra ANTES de que nadie scrollee un pixel, y el toque lo agarra un botón de la barra (tan alto como su propia superficie), no el control de la fila.

Un padding de contenido más grande no alcanza: sigue siendo la misma trampa con una lista un poco más larga. La solución es reservar el alto de la barra como `paddingBottom` del CONTENEDOR (el `style` de `ScreenContainer`, no su `contentStyle`) -- así el `ScrollView` nunca llega a dibujar nada detrás de la barra, sea cual sea el largo de la lista. `PantallaConTabs.tsx` ya lo resuelve así para toda pantalla que lo use; si una pantalla nueva arma su propio scroll sin pasar por ahí, tiene que reservar ese mismo alto (`ALTO_TAB_BAR + insets.bottom`) en su propio contenedor, no en el contenido.
